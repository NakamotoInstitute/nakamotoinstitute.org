---
date: Originally published in 2004
---
...
<p>Scarce objects are not a complete model of computation across trust boundaries. Indeed, there are many <a href="/the-idea-of-smart-contracts/">smart contracts</a> that can be implemented with cryptographic protocols and/or secure hardware but not with scarce objects. What scarce objects provide is a straightforward basis for implementing, in an intuitively secure way, the anonymous commodity exchange economies formalized in microeconomics in a P2P fashion on the Internet.</p>
...
<p>It is a design option whether to combine the Issuer and TA into a single server (thereby reduce exposure of the private key) or keep them separate (thereby enable certain personell controls based on <a href="/designing-trusted-services-with-group-controls/">separation of duties</a>). Distributed servers, described below, are an even better way to increase the trustworthiness of Issuers and TAs.</p>
...
<ul>
  <li>Expression of preferences – and their translation into codified expectations, or rights and duties – with both user-friendly GUIs ("source language") and a <a href="/contract-language/">formal contract language</a> ("target language"), and translation between them.</li>
...
<p>The problem of "translating" between bearer contracts (which represent and secure rights to scarce objects) can be cast as a problem of translating between <a href="/shelling-out/">monetary currencies</a>. For our purposes, a "currency" in a scarce object economy is simply any kind of <a href="/contracts-with-bearer">bearer contract used for holding and transferring wealth, rather than for consumption by the holder. It is thus a <a href="/shelling-out/#attributes-of-collectibles">"collectible"</a> (or "intermediate commodity", to use Carl Menger’s term). The market translator, incidentally, makes the O(n^2) prices in a barter economy, versus O(n) in a monetary economy, a far less important distinction.</p>
...
<p>One mode in which the MT can be used is to have Bob offer a take-it-or-leave-it binary contract, corresponding to the current retail practice of take-it-or-leave-it prices. In the mode pictured above, Alice and Bob negotiate back and forth. The negotation of the source contract terms will usually be manual. The "source language" will typically be a human readable GUI, while the "target" will be a standard <a href="/contract-language/">formal contract language</a>. If Alice and Bob can input preferences leading to automated negotiations, then a "shopping bot" and "catalog bot" respectively may be used. This is a layer above and beyond the scope of the MT. The MT is only a "shopping bot" in the restricted but important area of contracts composed of atomic bearer contracts – rights to scarce objects – to the extent that the price relationships between these bearer contracts are available from quoted markets.</p>
