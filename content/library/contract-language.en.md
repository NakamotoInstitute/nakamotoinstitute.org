...
<!--
<p>Next we use our language to sketch out an idea for something that I won't claim bears much resemblance to a contract at all. It nevertheless bears some relationship to contracts, and in particular to property title transfers and the relationship between a property owner and the provider of security services to that property. Furthermore, security services and technologies are important for both enforcing the performance of contracts and protecting this performance from interfering third parties.</p>

<p>The idea is for a security system to protect a building. Our hypothetical device learns who the buildings owner is from a public title database (see article securetitle.html). Lacking such a service it could have the owner or security service reprogrammed into it specifically by a new owner (but we don't show that design). Our system looks up which security service the owner has hired, and notifies them of the security event. If the property is sold, the software automatically figures out, from <strong>getTitle()</strong>, who the new owner is, and which security service is their authorized agent for handling security events. <strong>passEvent()</strong> passes on the event information as it was received by the current event.</p>
...
<p>We might save precious seconds of delay in notifying security by having a property title transfer and change of security service generate events, then storing <strong>securityService</strong> as a state variable. This way the security system doesn't have to take time to look up the owner and then the security service during a break-in.</p>
...
<pre>
-- Specific keywords after each instruction that distinguish it
as:
- asynchronous (also)
- asynchrononous and atomic (with)
Since this is not guarunteed atomic, is the only sure property
that both instructions must be complete before we move on?  In
this case "X with Y" is equivalent to "X also Y then".
- synchronous (then).  this also blocks all equally or lower
nested prior asynchronous calls.
-- My messages are only-way -- they have no return arguments.
(But then again, the whole point is to swap rights and perform
services defined outside the scope of the language.  Actual
"information processing" should be done using another language).
-- E allows "then" by defualt.  An asynch "also"-style call must have
an explicit "when" to wait for its return.  In my language
these calls block at the next equally or higher nested "then"
and there are no return arguments.  E has no equivalent of
"with", probably becomes atomicity does not have proven
reliability and doing "with" across trust boundaries
(as usually done here)  requires a trusted escrow or,
perhaps, some very weird crypto protocol.
-- Events are asynch-callable methods that are active only
when "when" is in the "waiting" state.  (Catch can also
be in a "inactive" or "active" state).  They search for whenes
starting from the leaves of the target named clause ("throw at target").
E doesn't have anything like this, as far as I know.  Would
break capability discipline?  (But my intention is to implement
all atoms securely -- surely there is some subset of events
that can be sent securely, i.e. by never sending capabilities
in their message args?)
</pre>
-->
